    ...
    while len(con) != 0:
        ...
        for var in variable:
            for dom in var.cur_domain():
                if not c.has_support(var, dom):
                    if var.in_cur_domain(dom):
                        var.prune_value(dom)
                        returnlist.append((var,dom))
                        
                    if var.cur_domain_size() == 0:
                        return False, returnlist

                    cons2 = csp.get_cons_with_var(var)
                    for a in cons2:
                        if a not in con:
                            con.append(a)
